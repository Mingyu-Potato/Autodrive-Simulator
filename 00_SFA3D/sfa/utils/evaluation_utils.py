...
from utils.evaluation_iou import calculate_iou
...
def draw_compared_image(configs, img, detections, labels):
        ...
        if len(detections[j]) > 0:
            for det in detections[j]:
                ...
                for box_idx, (cls_id, x, y, z, h, w, l, yaw) in enumerate(labels):
                    ...
                    det_iou_2 = calculate_iou(configs, detection_input, ground_truth_input)
                    if det_iou_2 > max_det_iou_2:
                        max_det_iou_2 = det_iou_2

                if max_det_iou_2 >= 0.5:
                    drawRotatedBox(img, _x, _y, _w, _l, _yaw, [0,0,255]) # 빨간 박스
                else:
                    drawRotatedBox(img, _x, _y, _w, _l, _yaw, [0,255,0]) # 초록 박스
        ...
        for j in range(configs.num_classes):
            if len(detections[j]) > 0:
                for det in detections[j]:
                    ...
                    gt_iou_2 = calculate_iou(configs, detection_input, ground_truth_input)
                    if gt_iou_2 > max_gt_iou_2:
                        max_gt_iou_2 = gt_iou_2

        if max_gt_iou_2 < 0.5:
            drawRotatedBox(img, x1, y1, w1, l1, yaw, [255,0,0]) # 파랑 박스
